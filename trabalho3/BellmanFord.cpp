...
struct graph {
    ...
    float BellmanFord_time (int s) {
        ...
        createFile_dij("Belman", {dist, parent, level}, dt);
...
int main() {
    ...
    while (infile >> u >> v >> w) {
        edges.push_back({(float)u, (float)v, w});
    }
    ...
    // weightened = 1
    graph g(edges, n, m, directed, 1, 1); 


    // Detectar se há peso negativo
    ...
    */
    ...
    cout << "Pais:\n";
    for (int i = 0; i < n; i++) {
        cout << "Vertice " << i << ": " << parent[i] << '\n';
    }

    cout << "Niveis:\n";
    for (int i = 0; i < n; i++) {
        cout << "Vertice " << i << ": " << level[i] << '\n';
    }
